...
// ── Main component ─────────────────────────────────────────────────────────────
export default function EstateCOP() {
  ...
  const currentEstate = userObj?.estate || null;

  // ── Initialize Default Selection ─────────────────────────────────────────────
  useEffect(() => {
    const currentMonth = new Date().toISOString().slice(0, 7); // "YYYY-MM"
    setSelected({ code: null, month: currentMonth });
  }, []);

  // ── Fetch report data ────────────────────────────────────────────────────────
  const fetchData = useCallback(async (sel) => {
    if (!sel) return;
    setStatus("loading");
    try {
      const res = await apiClient.get(`/estate-cop/report?code=${encodeURIComponent(sel.code)}&month=${encodeURIComponent(sel.month)}`);
      if (res.success) {
        setData(res.data);
        setStatus("ok");
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => fetchData(selected), REFRESH_MS);
  ...
  const exportCSV = () => {
    ...
    const headers = ['Description', `${data.month} Amount (LKR)`, 'Cost Ave. (LKR/Kg)', 'To Date Amount (LKR)', 'To Date Cost Ave. (LKR/Kg)'];
    const rows = [];
    
    // Estate Operation
    rows.push(['ESTATE OPERATION', '', '', '', '']);
    rows.push([`Crop (Kg) – ${currentEstate || data.estate}`, data.crop.monthly, '', data.crop.todate, '']);
    rows.push([
      `ESTATE LEAF INCOME – ${currentEstate || data.estate}`,
    ...
    const csvContent = [
      ['Estate', currentEstate || data.estate],
      ['Period', data.month],
      [],
      headers.join(','),
    ...
    link.href = url;
    link.setAttribute('download', `Monthly_COP_Report_${data.month}.csv`);
  ...
  const exportPDF = () => {
    ...
    doc.text(`${currentEstate || data.estate}`, 14, 15);
    doc.setFontSize(10);
    doc.setFont('Helvetica', 'normal');
    doc.text('MONTHLY COP REPORT', 14, 21);
    doc.text(`Period: ${data.month}`, 14, 27);
    ...
    tableBody.push([`Crop (Kg) – ${currentEstate || data.estate}`, fmt(data.crop.monthly), '', fmt(data.crop.todate), '']);
    tableBody.push([
      `ESTATE LEAF INCOME – ${currentEstate || data.estate}`,
    ...
    autoTable(doc, {
      startY: 40,
      head: [['Description', `${data.month}`, 'Cost Ave.', 'To Date', 'Cost Ave.']],
      body: tableBody,
      theme: 'grid',
    ...
    doc.save(`Monthly_COP_Report_${data.month}.pdf`);
    setShowExportOptions(false);
  };
  ...
  const reportsByYear = useMemo(() => {
    ...
    reports.forEach(r => {
      ...
      const year = parts.length === 2 ? `20${parts[1]}` : 'Other';
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1 flex items-center gap-2">
            Detailed Operation Reports & Cost Analysis
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative group flex items-center">
            ...
            <input
              type="month"
              value={selected?.month?.includes('-') && selected.month.length === 7 ? selected.month : ""}
              onChange={e => {
                if (e.target.value) {
                  setSelected({ code: null, month: e.target.value });
                }
              }}
      ...
      {derived && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="premium-card">
            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">Total Monthly Income</p>
          ...
          <div className="premium-card">
            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">Total Monthly Expenses</p>
            <div className="flex items-center justify-between">
          ...
          <div className="premium-card">
            <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">Net Profit (Monthly)</p>
            <div className="flex items-center justify-between">
        ...
        <div className="premium-card overflow-hidden p-0 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl">
          <div id="report" className="p-8 md:p-12 max-w-5xl mx-auto">
            ...
            <div className="text-center mb-10 border-b-4 border-slate-900 dark:border-slate-700 pb-6">
              <h2 className="text-2xl font-black tracking-[0.3em] text-slate-900 dark:text-white uppercase mb-1">{currentEstate || data.estate}</h2>
              <h3 className="text-sm font-black tracking-widest text-slate-500 dark:text-slate-400 uppercase mb-4 italic">MONTHLY COP REPORT</h3>
              <div className="flex justify-center items-center gap-6 text-[11px] font-black uppercase tracking-widest text-slate-400">
                ...
                <span className="flex items-center gap-2"><Calendar size={12} /> Period: {data.month}</span>
              </div>
            </div>
                ...
                <table className="w-full border-collapse">
                  ...
                  <thead>
                    <tr className="border-b-2 border-slate-900 dark:border-slate-700">
                      ...
                      <th className="text-right px-3 py-2 text-[10px] font-black uppercase tracking-widest text-slate-400">{data.month}</th>
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    <tr className="bg-slate-50/50 dark:bg-slate-800/30">
                      <TL bold>Crop (Kg) – {currentEstate || data.estate}</TL>
                      <TR v={data.crop.monthly} bold /><td />
                      <TR v={data.crop.todate}  bold /><td />
                    </tr>
                    <tr className="bg-emerald-50/30 dark:bg-emerald-900/10">
                      <TL bold className="text-emerald-700">ESTATE LEAF INCOME – {currentEstate || data.estate}</TL>
      ...
      ) : status === 'error' ? null : status === 'ok' && reports.length === 0 ? (
        <div className="p-20 text-center premium-card">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">No reports found. Please seed the database with estate COP data.</p>
        </div>
      ) : (
        <div className="p-20 text-center premium-card">
