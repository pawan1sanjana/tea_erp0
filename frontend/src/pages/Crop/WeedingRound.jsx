...
import { 
  ...
  ChevronDown, AlertCircle, RefreshCcw, Shovel, Clock
} from 'lucide-react';
...
const WEEDING_INTERVAL_DAYS = AGRONOMIC_CONFIG.WEEDING.INTERVAL_DAYS;
...
export default function WeedingRound() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (resBlocks.success) {
        ...
        cutoffDate.setDate(cutoffDate.getDate() - WEEDING_INTERVAL_DAYS);
        ...
        mappedFields.forEach(f => {
          ...
          const tot = { id: f.id, area: 0, labours: 0, types: new Set(), latestRoundArea: 0, readinessArea: 0 };
          ...
          if (logs.length > 0) {
            ...
            d1.setDate(d1.getDate() + WEEDING_INTERVAL_DAYS);
            tot.nextDue = d1.toISOString().split('T')[0];

            const d2 = new Date(earliestDateOfLatestRound);
            d2.setDate(d2.getDate() + (WEEDING_INTERVAL_DAYS * 2));
            ...
            logs.forEach(log => {
              ...
              dataMap[f.id][dateKey] = {
                type: WEED_TYPES.find(t => t.id === log.type)?.label || log.type,
              ...
              // Readiness: Sum area weeded within the interval window
              if (logDate >= cutoffDate) {
                tot.readinessArea += Number(log.area) || 0;
              }
              
              // Current Round Progress (Label specific)
              if (log.round_label === latestRoundLabel) {
                tot.latestRoundArea += Number(log.area) || 0;
              }
            });
            
            // Cap readinessArea at block size
  ...
  const handleSync = async () => {
    if (!activeField || !syncData.area) return;
    setSyncing(true);
    try {
      const res = await apiClient.post('/crop/weeding-logs/entry', {
        block_id: activeField,
        date: syncData.date,
        type: syncData.type,
        area: syncData.area,
        round_label: syncData.round,
        chemical_type: syncData.chemical_type,
        chemical_qty: syncData.chemical_qty,
        recorded_by: 'Round Monitor'
      });
      if (res.success) {
        setShowSync(false);
        loadData();
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, `weeding_round_report_${year}_${month}.xlsx`);
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(20);
    doc.text("Weeding Round Operational Report", 14, 22);
    doc.setFontSize(11);
    doc.setTextColor(100);
    doc.text(`Generated on: ${new Date().toLocaleDateString()} | Year: ${year}`, 14, 30);
    ...
    const tableData = fields.map(f => {
      ...
      return [
        f.label,
        f.acres,
        (tot.latestRoundArea || 0).toFixed(2) + ' Ac',
        tot.nextDue || 'Ready',
    ...
    autoTable(doc, {
      startY: 40,
      head: [['Block', 'Total Area', 'Round Coverage', 'Next Due', 'Status']],
      body: tableData,
      theme: 'grid',
      headStyles: { fillColor: [5, 150, 105] }
    });
    
    doc.save(`weeding_report_${year}.pdf`);
    setShowExportOptions(false);
  };
    ...
    <div className="space-y-8 pb-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white font-outfit tracking-tight">
            Weeding Round <span className="text-emerald-600">Monitor</span>
          </h1>
          <p className="text-slate-500 text-sm font-medium flex items-center gap-2 mt-1">
            <RefreshCcw size={14} className="text-emerald-500" />
            Dependency-driven vegetation control & field preparation
          </p>
        </div>
          ...
          <div className="relative">
            ...
            {showExportOptions && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-2xl z-[100] p-2 animate-in slide-in-from-top-2">
                ...
                <button onClick={exportToPDF} className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black uppercase tracking-widest text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors text-left">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { 
            label: "Chemical Overhead", 
            val: Object.values(fieldData).reduce((s, f) => s + Object.values(f).reduce((s2, r) => s2 + (Number(r.chem_qty) || 0), 0), 0).toFixed(0), 
            unit: "Ltrs", 
            icon: Activity, 
          ...
          { 
            ...
            unit: "blocks", 
            icon: AlertCircle, 
            color: fields.filter(f => getDueStatus(f.id) === 'overdue').length > 0 ? "text-rose-500" : "text-emerald-500", 
            gradient: fields.filter(f => getDueStatus(f.id) === 'overdue').length > 0 ? "from-rose-500/20 to-rose-500/5" : "from-emerald-500/20 to-emerald-500/5" 
          },
        ].map((s, i) => (
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Field Preparation Status (Weeding)</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {fields.map(field => {
            ...
            const remaining = Math.max(0, totalAcres - readinessArea);
            const status = getDueStatus(field.id);

            const colorClass = status === "overdue" ? "border-rose-500/30 bg-rose-500/5 shadow-rose-500/5" : status === "warn" ? "border-amber-500/30 bg-amber-500/5" : "border-emerald-500/30 bg-emerald-500/5";
            const dotClass = status === "overdue" ? "bg-rose-500 shadow-[0_0_12px_rgba(244,63,94,0.6)] animate-pulse" : status === "warn" ? "bg-amber-500" : "bg-emerald-500";
            const textClass = status === "overdue" ? "text-rose-600" : status === "warn" ? "text-amber-600" : "text-emerald-600";

            return (
              <div key={field.id} onClick={() => setActiveField(field.id)} className={`premium-card p-4 border-2 ${colorClass} transition-all hover:scale-[1.02] group cursor-pointer ${activeField === field.id ? 'ring-2 ring-emerald-500 ring-offset-2 dark:ring-offset-slate-900' : ''}`}>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-end">
                    <div>
                      ...
                      <p className={`text-[11px] font-black font-outfit italic text-emerald-600`}>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center text-[8px] font-black uppercase tracking-widest">
                      <span className="text-slate-400">Field Readiness</span>
          ...
          <div className="premium-card p-0 overflow-hidden">
            <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Weeding Application Lifecycle</p>
              </div>
              <div className="text-right">
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                      {HUNDRED_TWENTY_DAYS.slice(colIdx * 30, (colIdx + 1) * 30).map(dateKey => {
                        ...
                        return (
                          <tr key={dateKey} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${row ? 'bg-emerald-50/30 dark:bg-emerald-900/10' : ''} ${isNextDue ? 'ring-2 ring-inset ring-amber-500/50 bg-amber-500/5' : isToday ? 'bg-indigo-500/5' : ''}`}>
                            ...
                            <td className="px-3 py-2">
                              {row ? (
                                <div className="flex flex-col leading-none">
                                   <span className="text-[9px] font-black uppercase text-emerald-600 truncate max-w-[80px]">
                                     {row.round || "Round 1"}
                                   </span>
