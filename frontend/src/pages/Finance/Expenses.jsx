...
import { ReceiptText, Plus, RefreshCcw, AlertTriangle, Wallet, Search, Download, FileText, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function Expenses() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (aRes.success) {
        setAccounts(aRes.data.filter(a => a.isActive));
        // Preselect General Expenses (5000) if exists
        const general = aRes.data.find(a => a.code === '5000');
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white font-outfit tracking-tight italic">Expenses Intelligence</h1>
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">
            Capture Operational Expenses & Automated Journaling
          </p>
        </div>
        ...
        <div className="premium-card p-5">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div className="text-right w-full md:w-auto">
              ...
              <p className="text-sm font-black text-slate-900 dark:text-white">{totalShown.toFixed(2)}</p>
            </div>
          </div>
        </div>
