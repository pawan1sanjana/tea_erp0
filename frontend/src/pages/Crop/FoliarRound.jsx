...
import { 
  Droplets, Calendar, BarChart3, Activity, Users,
...
const FOLIAR_INTERVAL_DAYS = AGRONOMIC_CONFIG.FOLIAR.INTERVAL_DAYS;
...
export default function FoliarRound() {
  const now = new Date();
  ...
  const loadData = async () => {
    ...
    try {
      const [resBlocks, resYearLogs] = await Promise.all([
        ...
        apiClient.get(`/crop/foliar-logs/year?year=${year}`)
      ]);
      ...
      if (resBlocks.success) {
        ...
        cutoffDate.setDate(cutoffDate.getDate() - FOLIAR_INTERVAL_DAYS);
        ...
        mappedFields.forEach(f => {
          ...
          if (logs.length > 0) {
            ...
            d1.setDate(d1.getDate() + FOLIAR_INTERVAL_DAYS);
            tot.nextDue = d1.toISOString().split('T')[0];

            const d2 = new Date(earliestDateOfLatestRound);
            d2.setDate(d2.getDate() + (FOLIAR_INTERVAL_DAYS * 2));
    ...
    <div className="space-y-8 pb-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white font-outfit tracking-tight">
            Foliar Round <span className="text-sky-600">Monitor</span>
          </h1>
          <p className="text-slate-500 text-sm font-medium flex items-center gap-2 mt-1">
            <RefreshCcw size={14} className="text-sky-500" />
            Micronutrient & pest control spray cycle tracking
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { 
            ...
            unit: "blocks", 
            icon: AlertCircle, 
            color: fields.filter(f => getDueStatus(f.id) === 'overdue').length > 0 ? "text-rose-500" : "text-sky-500", 
            gradient: fields.filter(f => getDueStatus(f.id) === 'overdue').length > 0 ? "from-rose-500/20 to-rose-500/5" : "from-sky-500/20 to-sky-500/5" 
          },
        ].map((s, i) => (
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Field Application Status (Foliar)</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {fields.map(field => {
            ...
            const colorClass = status === "overdue" ? "border-rose-500/30 bg-rose-500/5 shadow-rose-500/5" : status === "warn" ? "border-amber-500/30 bg-amber-500/5" : "border-sky-500/30 bg-sky-500/5";
            const dotClass = status === "overdue" ? "bg-rose-500 shadow-[0_0_12px_rgba(244,63,94,0.6)] animate-pulse" : status === "warn" ? "bg-amber-500" : "bg-sky-500";
            const textClass = status === "overdue" ? "text-rose-600" : status === "warn" ? "text-amber-600" : "text-sky-600";

            return (
              <div key={field.id} onClick={() => setActiveField(field.id)} className={`premium-card p-4 border-2 ${colorClass} transition-all hover:scale-[1.02] group cursor-pointer ${activeField === field.id ? 'ring-2 ring-sky-500 ring-offset-2 dark:ring-offset-slate-900' : ''}`}>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-end">
                    <div>
                      ...
                      <p className={`text-[11px] font-black font-outfit italic text-sky-600`}>
      ...
      <div className="grid grid-cols-1 gap-8">
        <div className="w-full">
          <div className="premium-card p-0 overflow-hidden border-2 border-sky-500/10">
            <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] font-black text-sky-600 uppercase tracking-widest">Foliar Application Lifecycle</p>
              </div>
              <div className="text-right">
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                      {HUNDRED_TWENTY_DAYS.slice(colIdx * 30, (colIdx + 1) * 30).map(dateKey => {
                        ...
                        return (
                          <tr key={dateKey} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${row ? 'bg-sky-50/30 dark:bg-sky-900/10' : ''} ${isNextDue ? 'ring-2 ring-inset ring-sky-500/50 bg-sky-500/5' : isToday ? 'bg-sky-500/5' : ''}`}>
                            <td className="px-3 py-2">
                               <div className="flex flex-col">
                                  <span className={`text-[10px] font-black uppercase tracking-tighter ${isNextDue ? 'text-sky-600' : isToday ? 'text-sky-600' : 'text-slate-700 dark:text-slate-200'}`}>
                                    {d.toLocaleDateString('en-US', { day: '2-digit', month: 'short' })}
                                  </span>
                                  {isToday && <span className="text-[7px] font-black text-sky-500 uppercase tracking-widest leading-none">Today</span>}
                               </div>
                            </td>
                            <td className="px-3 py-2">
                              {row ? (
                                <div className="flex flex-col leading-none">
                                   <span className="text-[9px] font-black uppercase text-sky-600 truncate max-w-[80px]">
                                     {row.round || "Round 1"}
                                   </span>
                                ...
                                <div className="flex items-center gap-1">
                                   ...
                                   <span className="text-[8px] font-black uppercase text-sky-600 italic tracking-tighter">
                            ...
                            <td className="px-3 py-2 text-right">
                              <span className={`text-[10px] font-black font-outfit ${row?.area ? 'text-sky-600 dark:text-sky-400' : 'text-slate-300 dark:text-slate-800'}`}>
